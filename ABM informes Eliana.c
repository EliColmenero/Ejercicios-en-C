...
void mostrarCantidadDeAlumnosPorCarrera(eAlumno alumnos[],
                                        ...
                                        eCarrera carreras[],
                                        int tamC);
int amanteMilanesas(int idCarrera, eAlmuerzo almuerzos[], eAlumno alumnos[], int tamAlmuerzos, int tamAlumnos);

void carreraMasInscriptos(eAlumno alumnos[],
                          int tam,
                          eCarrera carreras[],
                          int tamC);
                          void facturarAlmuerzoAlumno(eAlumno alumnos[],
                                        int tamAlmunos, eCarrera carreras[], int tamC,
                                        eComida comidas[], tamComidas, eAlmuerzo almuerzos[], int tamAlmuerzos);

void facturarAlmuerzoAlumno(eAlumno alumnos[], int tamAlmunos, eCarrera carreras[],
                            int tamC, eComida comidas[],
                            tamComidas, eAlmuerzo almuerzos[], int tamAlmuerzos);
...
void mostrarInformes(eAlumno alumnos[], int tam, eCarrera carreras[], int tamC)
{
    ...
    do
    {
        ...
        case 12:
            amanteMilanesas(idCarrera, almuerzos, alumnos, tam, tam);

        case 13:
            facturarAlmuerzoAlumno(alumnos, tamAlmunos, carreras,
                            tamC, comidas, tamC, almuerzos, tam);
            break;
...
//Esta funcion cuenta las milanesas que se comieron en cada carrera
int amanteMilanesas(int idCarrera, eAlmuerzo almuerzos[], eAlumno alumnos[], int tamAlmuerzos, int tamAlumnos){
    int i;
    int ContAl = 0;

    //Cada for recorre un vector, el primero el de alumnos y el segundo el de almuerzos
    for(i = 0; i < tamAlumnos; i++){ //Este for me va a decir que voy a buscar
            if(alumnos[i].isEmpty == 0 && alumnos[i].idCarrera == idCarrera){
        for(j = 0; j < tamAlmuerzos; j++){ //Este for me va a decir donde voy a buscar
...
void CarreraAmanteMilanesas(eCarrera carreras[], int tamCarreras, ){
    int i;
    int milanesas[tamCarreras];

    for(i = 0; i < tamCarreras; i++){
        milanesas[i] = amanteMilanesas(carreras[i].id, almuerzos[], alumnos[], tamAlmuerzos, tamAlumnos);
    }
}

void facturarAlmuerzoAlumno(eAlumno alumnos[], int tamAlmunos, eCarrera carreras[], int tamC, eComida comidas[], tamComidas, eAlmuerzo almuerzos[], int tamAlmuerzos){
    float total = 0;

    //Informe: informe deuda de alumno seleccionado

    system("cls");
    printf("******Facturacion alumno******* \n");

    mostrarAlumnos(alumnos, tamAlmunos, carreras, tamC);
    ...
    for(i=0; i<tamAlmuerzos; i++){
        if(almuerzos[i].legajo == legajo){
            for(i=0;j<tamC;j++){
                if(comidas[j].id == almuerzos[i].idComida == idComidas){
                    total += comidas[j].precio;
                }  //cada vez que encuentro el id de la comida acumulo el precio
            }
        }
    }
...
void recaudacionXalgunaComida(eComida comidas[], int){
float total = 0;
float precio;
int idComida;
